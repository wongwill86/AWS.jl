...
# Generate for EC2
const wsdl = EC2_WSDL(xp_parse(open(readall, "./wsdl/ec2_2013_02_01.wsdl")))

macro gencall(operation::String)
    operation = wsdl.xml[xpath"portType/operation[@name=\"$(operation)\"]"]
    rqst = wsdl.xml[xpath"types/xs:schema/xs:complexType[@name=\"$(action)ResponseType\"]"]
    response = wsdl.xml[xpath"types/xs:schema/xs:complexType[@name=\"$(action)ResponseType\"]"]

    quote
        function $(operation)(env::AWSEnv, msg::$(operation)Type)
            ec2resp::EC2Response = call_ec2(env, "$(operation)" , msg)
            if  (ec2resp.pd != nothing) && (ec2resp.obj == nothing)
                ec2resp.obj = $(operation)ResponseType(ec2resp.pd)
            end
            ec2resp
        end
        function $(operation)(env::AWSEnv; kwargs...)
            msg=$(operation)Type()
            for p in kwargs
...
# Generate required types and functions on the fly
